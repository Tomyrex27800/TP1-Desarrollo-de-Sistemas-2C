...
query_vista_por_region = "SELECT a.region, COUNT(DISTINCT b.country_code) AS paises_con_sedes, SUM(DISTINCT c.gdp_pc_2023_usd) AS promedio_pbi_2023 FROM df_paises a LEFT JOIN df_sedes b ON a.country_code = b.country_code LEFT JOIN df_gdp_per_capita_2023 c ON a.country_code = c.country_code GROUP BY a.region"
